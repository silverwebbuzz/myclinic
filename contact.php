...
.ct-page .wrap { max-width: 1040px; margin: 0 auto; padding: 0 24px; }
.ct-grid {
  display: grid;
...
.ct-lede { font-size: 15.5px; line-height: 1.6; color: var(--ink-2); margin: 0 0 28px; max-width: 420px; }
.ct-channels { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 16px; }
.ct-channels li { display: flex; gap: 13px; align-items: center; }
.ct-ch-ic {
  width: 42px; height: 42px; border-radius: 12px; flex-shrink: 0;
  background: #fff; border: 1px solid var(--line);
  display: grid; place-items: center; font-size: 19px;
}
.ct-channels b { display: block; font-size: 12px; font-weight: 600; color: var(--mute); text-transform: uppercase; letter-spacing: 0.05em; }
.ct-channels a { font-size: 15px; font-weight: 600; color: var(--ink); text-decoration: none; }
.ct-channels a:hover { color: var(--teal-700); }
...
.ct-fld { display: flex; flex-direction: column; gap: 6px; flex: 1 1 200px; min-width: 0; }
.ct-fld > span { font-size: 11px; font-weight: 600; letter-spacing: 0.06em; text-transform: uppercase; color: var(--mute); }
.ct-fld > span em { font-style: normal; text-transform: none; letter-spacing: normal; color: var(--teal-700); font-weight: 500; }
...
.ct-fld textarea { resize: vertical; }
.ct-fld input:focus, .ct-fld select:focus, .ct-fld textarea:focus {
  border-color: var(--teal-400); box-shadow: 0 0 0 3px rgba(15,155,110,0.14);
}
.ct-fld input:disabled, .ct-fld textarea:disabled, .ct-fld select:disabled { background: var(--bg-2); opacity: 0.7; }
.ct-captcha { display: flex; }
...
.ct-submit { padding: 14px 22px; font-size: 15px; font-weight: 600; border-radius: 12px; }
.ct-note { font-size: 12.5px; color: var(--mute); margin: 0; line-height: 1.5; text-align: center; }
.ct-note a { color: var(--teal-700); text-decoration: underline; }

/* Success */
.ct-success { text-align: center; padding: 24px 8px; }
.ct-success-ic { font-size: 44px; margin-bottom: 10px; }
.ct-success h2 { font-size: 22px; font-weight: 600; margin: 0 0 8px; letter-spacing: -0.4px; }
.ct-success p { font-size: 15px; color: var(--ink-2); line-height: 1.6; margin: 0 auto 20px; max-width: 360px; }
.btn-outline {
...
function contactForm(captchaEnabled) {
  return {
    ...
    async submit() {
      ...
      if (!this.form.email.trim()) { this.errorMsg = 'Please enter your email.'; return; }
      if (this.form.message.trim().length < 10) { this.errorMsg = 'Please write a bit more so we can help.'; return; }
      ...
      try {
        const r = await fetch('/api/contact?action=submit', {
          ...
          body: JSON.stringify({ ...this.form, 'g-recaptcha-response': captcha || undefined }),
        });
        const text = await r.text();
        let j; try { j = JSON.parse(text); } catch (e) {
          this.errorMsg = 'Server returned an unexpected response (HTTP ' + r.status + ').'; return;
        }
        if (!j.ok) { this.resetCaptcha(); this.errorMsg = this.errorText(j.error); return; }
    ...
    errorText(code) {
      switch (code) {
        ...
        case 'message_too_short': return 'Please write a bit more so we can help.';
        case 'message_too_long':  return 'That message is a little too long — please shorten it.';
        case 'captcha_failed':    return 'Captcha check failed. Please try again.';
        case 'send_failed':       return "We couldn't send your message right now. Please email hello@eclinicpro.com directly.";
        case 'method_not_allowed':
        case 'unknown_action':    return 'Something went wrong submitting the form. Please refresh and try again.';
