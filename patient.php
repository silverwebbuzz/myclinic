    ...
    <div class="wrap">
      ...
      <div class="pt-grid is-stacked">
        ...
        <div class="pt-section pt-section-tabbed">
          ...
          <!-- Booking runs entirely inside the partner's portal below. Nothing
               here touches our lab_orders tables — the patient transacts with
               the partner directly, so the "Lab bookings" tab only ever lists
               orders placed through our own (legacy) storefront. -->
          <div x-show="tab === 'labbook'" class="pt-tab-pane">
            ...
            <!-- The frame is a fixed 1480px box; this wrapper scrolls it
                 sideways when the panel column is narrower, so the partner
                 layout is never squeezed into a width it can't handle. -->
            <div class="pt-labframe-scroll">
...
<style>
  ...
  /* -------- Embedded partner booking portal --------
     Fixed 1480x1100 on desktop: the partner's layout needs roughly this much
     room, and a fixed box is predictable. The pane scrolls horizontally when
     the panel column is narrower, so the frame is never squeezed. */
  .pt-labframe-scroll {
    overflow-x: auto;
    overflow-y: hidden;
    -webkit-overflow-scrolling: touch;
...
<script>
  ...
  function patientPanel(isLoggedIn) {
    return {
      ...
      // the one most patients arrive for.
      tab: 'labbook',
      // The partner iframe mounts lazily: src is only set once the tab is
      // opened, so a patient who never books never pays for that page load.
      labFrameLoaded: false,
      labFrameReady: false,
      // Partner booking portal. One constant so the pageId (which identifies
      // our account to the lab) lives in exactly one place.
