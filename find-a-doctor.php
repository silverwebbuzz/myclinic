...
$initialFilters = ecp_parse_find_doctor_filters($_GET, $seoMeta);
$initialFilters['page'] = max(1, (int) ($_GET['page'] ?? 1));
        ...
        <section class="fd-hero fd-desktop-only">
            <div class="wrap-wide">
                        ...
                        <div class="fd-ac" x-show="acOpen" @mousedown.prevent x-transition.opacity>
                            ...
                            <template x-if="filteredLocations().length === 0">
                                <div class="fd-ac-item" style="color: var(--mute);">
                                    <div class="ic">·</div>
                                    <div>
                                        <div class="nm">No matches</div>
                                        <div class="sb">Try a city or area name</div>
                                    </div>
                                </div>
                            </template>
                ...
                <div class="fd-specs" style="display:flex;align-items:center;gap:8px;">
                    <button type="button" class="fd-spec" :class="spec === 'all' ? 'active' : ''" @click="setSpec('all')">
                        All specialties
                    </button>
                            ...
                            <div class="fd-spec-list">
                                <template x-for="s in g.items" :key="s.id">
                                    <button type="button"
                                        ...
                                        @click="setSpec(s.id); specPanelOpen = false">
                                        <span x-text="s.label"></span>
                                    </button>
                                </template>
                            </div>
                    ...
                    <div class="fd-card">
                        ...
                        <div class="fd-identity">
                            <div class="fd-name-row">
                                <a class="fd-name" :href="profileUrl(d)" style="color:inherit;text-decoration:none" x-text="d.name"></a>
                        ...
                        <div class="fd-book">
                            ...
                            <div class="fd-actions">
                                ...
                                <template x-if="!d.gmaps_url && !d.website">
                                    <a :href="profileUrl(d)" class="fd-btn">View profile</a>
                                </template>
...
<script>
    ...
    function findDoctor() {
        return {
            ...
            specPanelOpen: false,
            avail: 'any',
            availOpen: false, // client-only (no API equivalent yet)
            video: false,
            gender: 'any',
            genderOpen: false, // client-only
            minRating: Number(window.FD_DATA.initialFilters.min_rating || 0),
            ratingOpen: false,
            lang: 'any',
            langOpen: false, // client-only
            ...
            loadFromUrl() {
                ...
                this.fetchPage(this.page, true).finally(() => {
            ...
            pageItems() {
                return this._applyClientFilters(this.doctors);
            },
            ...
            profileUrl(d) {
                return d.profile_url || '/find-a-doctor';
            },

            avatarBg(d) {
                if (d.photo_url && !d._photoBroken) {
                    return 'background-image:url(' + d.photo_url + ');background-size:cover;background-position:center;color:transparent';
                }
                return '';
            },

            // Brand default doctor avatar (200×200 PNG) used when a doctor has
            // no Google photo and no uploaded clinic logo.
            defaultAvatar(d) {
...
<script>
    function avatarComponent(photoUrl) {
        return {
            imageSrc: photoUrl,
            showImage: !!photoUrl,
